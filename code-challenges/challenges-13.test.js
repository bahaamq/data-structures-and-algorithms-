...
const firstLetters = (arr) => {
 ...
 const firstElement= arr.map((item)=>
  {
...
const findHappiness = (arr) => {
  // Solution code here...
  //Decllaring new Arr cuz if the condition is wrong it returns undefined and stored in the ortifginal array
  let newArr=[]
   arr.map((item)=>
    {
     if (item.includes(':)'))
...
let matches=''
 ...
 const lettersOnly= arr.map((item)=>
  {
     matches = item.replace(/[^0-9]+/g, "")
     //the ^ in this case means (not),, so non numerical will be replaced ..
  return matches
  })
